...
Runs `gofmt` check, `prettier` check, `go vet`, a build, `golangci-lint`, the `api.txt` drift check, and the race-enabled test suite. This is the same gate CI runs. Fix failures and rerun before committing; don't commit past a red gate. One-time setup to run this automatically on `git commit`: `task hooks:install`.
