...
- Calling `(*Future[T]).Get()` blocks until the task completes, fails, or the workflow context is canceled. A task that has already finished returns its result even if the run is being canceled at that moment.
